...
double *g_C;
double *g_D;
double elapsed[12], times[10];


#define TIME()	struct timespec start, finish; \
				clock_gettime(CLOCK_MONOTONIC, &start);

#define ENDTIME(i)	clock_gettime(CLOCK_MONOTONIC, &finish); \
...
int main(int argc, char** argv) {
	...
	int min = 0, max = 0;
	for(int i = 0; i < 12; i++) {
		if(elapsed[i] < elapsed[min])
			min = i;
		if(elapsed[i] > elapsed[max])
			max = i;
	}
	...
	if (world_rank == 0) {
		for (int i = 1; i < world_size; i++) {
			MPI_Recv(times + (world_size * sizeof(double)), 1, MPI_DOUBLE, i, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
		for(int i = 0; i < world_size; i++) {
			total_elapsed += times[i];
		}
	} else {
		MPI_Send(&total_elapsed, 1, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD);
	}
